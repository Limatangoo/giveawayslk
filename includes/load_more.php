...
if($click!=""){
   $trending_deals = "SELECT tittle,currency,old_price,discount_price,img1,link FROM deals WHERE trending='yes'";
	...
	$other_deals = "SELECT tittle,currency,old_price,discount_price,img1,link FROM deals WHERE trending='no'";
  ...
  while($i<4){
    ...
    while ($carousel_count<7) {
      ...
      <div class="hp-mod-price-first-line">
        <span class="currency">'.$carousel_1['currency'].'.</span><span class="price" id="load_discounted_price'.($click).'">'.$carousel_1['discount_price'].'</span>
      </div>
    
        <div class="hp-mod-price-second-line"><span class="hp-mod-price-text align-left">
      <span class="currency"></span><span class="price" id="load_old_price'.($click).'">'.$carousel_1['old_price'].'</span>
      </span>
      ...
      <div class="hp-mod-price-first-line">
        <span class="currency">Rs.</span><span class="price" id="load_discounted_price'.($click).'">'.$carousel_2['discount_price'].'</span>
      </div>
    
        <div class="hp-mod-price-second-line"><span class="hp-mod-price-text align-left">
      <span class="currency"></span><span class="price" id="load_old_price'.($click).'">'.$carousel_2['old_price'].'</span>
      </span>
      <span class="hp-mod-discount align-left"> -'.$percentage.'%</span></div>
      </div>
